...
import { deleteArticle, updateArticle } from '../../../../lib/services/articles';
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  const article = await updateArticle(runtime.env.DB, params.id || '', {
    title: parsed.title,
    slug: parsed.slug,
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  await deleteArticle(runtime.env.DB, params.id || '');
  return json({ ok: true });
};
