...
import { deleteCoreValue } from '../../../../../lib/services/values';
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  const input = contentType.includes('application/json')
    ...
    : {
        confirmValueId: await readFormDataValue(request, 'confirmValueId'),
        source: await readFormDataValue(request, 'source'),
      };
  const source = input.source === 'edit' ? 'edit' : 'list';

  if (input.confirmValueId !== valueId) {
    const message = 'Konfirmasi penghapusan core value tidak valid.';
  ...
  try {
    await deleteCoreValue(runtime.env.DB, {
    ...
    return new Response(null, { status: 302, headers: { Location: '/settings/values?success=Core%20value%20berhasil%20dihapus.' } });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Core value gagal dihapus.';
